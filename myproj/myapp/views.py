...
def editprofile(request):
	...
	lformset = LFormSet(request.POST)
	iformset = IFormSet(request.POST)
	uformset = UFormSet(request.POST)
	wformset = WFormSet(request.POST)
        print u.username, "in view"
        if form.is_valid() and lformset.is_valid() and iformset.is_valid() and uformset.is_valid() and wformset.is_valid():
	    print "valid"  
            u = form.save()
	    lformset.save()
	    iformset.save()
